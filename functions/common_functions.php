<?php
// Include security helpers - use absolute paths
...
function getProduct($numToDisplay = '')
{
    ...
    if (!isset($_GET['category'])) {
        if (!isset($_GET['brand'])) {
            empty($numToDisplay) ? $select_product_query = "SELECT * FROM `products` ORDER BY rand()" : $select_product_query = "SELECT * FROM `products` ORDER BY rand() LIMIT 0,?";
            
            if (empty($numToDisplay)) {
                $select_product_result = executeQuery($con, $select_product_query);
            } else {
                $select_product_result = executeQuery($con, $select_product_query, [$numToDisplay], "i");
            }
            
            if ($select_product_result) {
                while ($row = mysqli_fetch_assoc($select_product_result)) {
                    $product_id = escapeOutput($row['product_id']);
                    $product_title = escapeOutput($row['product_title']);
                    $product_image_one = escapeOutput($row['product_image_one']);
                    $product_price = escapeOutput($row['product_price']);
                    $category_id = escapeOutput($row['category_id']);
                    $brand_id = escapeOutput($row['brand_id']);
...
function filterCategoryProduct()
{
    ...
    if (isset($_GET['category'])) {
        ...
        $select_product_query = "SELECT * FROM `products` WHERE category_id = ?";
        $select_product_result = executeQuery($con, $select_product_query, [$category_id], "i");
...
function filterBrandProduct()
{
    ...
    if (isset($_GET['brand'])) {
        ...
        $select_product_query = "SELECT * FROM `products` WHERE brand_id = ?";
        $select_product_result = executeQuery($con, $select_product_query, [$brand_id], "i");
...
function getBrands()
{
    ...
    $select_brands_result = executeQuery($con, $select_brands_query);
    
    if ($select_brands_result) {
        while ($brands_row_data = mysqli_fetch_assoc($select_brands_result)) {
            $brand_title = escapeOutput($brands_row_data['brand_title']);
            $brand_id = escapeOutput($brands_row_data['brand_id']);
            echo "
        <li class='nav-item'>
...
function getCategories()
{
    ...
    $select_category_result = executeQuery($con, $select_category_query);
    
    if ($select_category_result) {
        while ($categories_row_data = mysqli_fetch_assoc($select_category_result)) {
            $category_title = escapeOutput($categories_row_data['category_title']);
            $category_id = escapeOutput($categories_row_data['category_id']);
            echo "
        <li class='nav-item'>
...
function search_product()
{
    ...
    if (isset($_GET['search_data_btn'])) {
        $search_data_value = sanitizeInput($con, $_GET['search_data']);
        $search_term = "%" . $search_data_value . "%";
        $search_product_query = "SELECT * FROM `products` WHERE product_title LIKE ? OR product_keywords LIKE ?";
        $search_product_result = executeQuery($con, $search_product_query, [$search_term, $search_term], "ss");
    ...
    if (isset($_GET['product_id'])) {
        if (!isset($_GET['category'])) {
            if (!isset($_GET['brand'])) {
                ...
                $select_product_result = executeQuery($con, $select_product_query, [$product_id], "i");
                
                if ($select_product_result && mysqli_num_rows($select_product_result) > 0) {
                    while ($row = mysqli_fetch_assoc($select_product_result)) {
                        $product_id = escapeOutput($row['product_id']);
                        $product_title = escapeOutput($row['product_title']);
                        $product_desc = escapeOutput($row['product_description']);
                        $product_image_one = escapeOutput($row['product_image_one']);
                        $product_image_two = escapeOutput($row['product_image_two']);
                        $product_image_three = escapeOutput($row['product_image_three']);
                        $product_price = escapeOutput($row['product_price']);
                        $category_id = escapeOutput($row['category_id']);
                        $brand_id = escapeOutput($row['brand_id']);
...
function cart($num_of_items = 1)
{
    if (isset($_GET['add_to_cart'])) {
        ...
        $getProductId = (int)$_GET['add_to_cart'];
        
        // FIXED: Table name changed from card_details to cart_details
        $select_query = "SELECT * FROM `cart_details` WHERE ip_address = ? AND product_id = ?";
        $select_result = executeQuery($con, $select_query, [$getIpAddress, $getProductId], "si");
        ...
        if ($select_result && mysqli_num_rows($select_result) > 0) {
            ...
            $insert_result = executeUpdate($con, $insert_query, [$getProductId, $getIpAddress], "is");
...
function cart_item()
{
    ...
    $select_result = executeQuery($con, $select_query, [$getIpAddress], "s");
    ...
    echo escapeOutput($count_cart_items);
}
...
function total_cart_price()
{
    ...
    echo escapeOutput(number_format($total_price, 2));
}
...
function get_user_order_details()
{
    ...
    $get_details_query = "SELECT * FROM `user_table` WHERE username = ?";
    $get_details_result = executeQuery($con, $get_details_query, [$username], "s");
    
    if ($get_details_result && mysqli_num_rows($get_details_result) > 0) {
        while ($row_query = mysqli_fetch_array($get_details_result)) {
            $user_id = (int)$row_query['user_id'];
            
            if (!isset($_GET['edit_account']) && !isset($_GET['my_orders']) && !isset($_GET['delete_account'])) {
                $get_orders_query = "SELECT * FROM `user_orders` WHERE user_id = ? AND order_status = 'pending'";
                $get_orders_result = executeQuery($con, $get_orders_query, [$user_id], "i");
                
                if ($get_orders_result) {
                    $row_orders_count = mysqli_num_rows($get_orders_result);
                    if ($row_orders_count > 0) {
                        $safe_count = escapeOutput($row_orders_count);
                        echo "<h3 class='text-center mb-3'>You have <span class='text-2'>$safe_count</span> pending orders</h3>
