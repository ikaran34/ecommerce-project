...
if (isset($_POST['admin_login'])) {
    ...
    $select_query = "SELECT * FROM `admin_table` WHERE admin_name='$username' OR admin_email='$username'";
    ...
    if (mysqli_num_rows($select_result) > 0) {
        ...
        if (password_verify($password, $row_data['admin_password'])) {
            ...
            echo "<script>alert('Invalid Credentials - Wrong Password')</script>";
        }
    } else {
        echo "<script>alert('Invalid Credentials - Username or Email not found')</script>";
    }
}
?>
